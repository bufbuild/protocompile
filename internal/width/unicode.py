...
# This file has been adapted by Buf for generating equivalent Go code.
# The license is as stated above.
...
func lookupWidth{cjk}(c rune) (byte, widthInfo) {{
    t1Offset := _WIDTH_ROOT{_cjk}[c >> {TABLE_SPLITS[1]}]
    ...
    t2Offset := _WIDTH_MIDDLE[int(t1Offset)][int(c) >> {TABLE_SPLITS[0]} & 0x{(2 ** (TABLE_SPLITS[1] - TABLE_SPLITS[0]) - 1):X}];
    ...
    packedWidths := _WIDTH_LEAVES[int(t2Offset)][int(c) >> 2 & 0x{(2 ** (TABLE_SPLITS[0] - 2) - 1):X}];
...
"""
    ...
    for (lo, hi), width in special_ranges:
        ...
        if hi == lo:
            ...
            s += f"c >= 0x{lo:X} && c <= 0x{hi:X}"

        if width.is_carried():
            width_info = width.name
        else:
            width_info = "DEFAULT"
        s += f" {{ return {width.width_alone()}, widthInfo_{width_info} }}\n"

    s += f"""    return 2, widthInfo_EMOJI_PRESENTATION
}}

func singleCharWidth{cjk}(c rune) int {{
    if c < 0x7F {{
        if c >= 0x20 {{
...
func widthInStr{cjk}(c rune, nextInfo widthInfo) (byte, widthInfo) {{
    ...
    if c <= 0xA0 {
        switch c {
            ...
            case '\\r': 
                if nextInfo == widthInfo_LINE_FEED { return 0, 0 }
                fallthrough
            default: return 1, 0
        }
    } else {
    ...
    for lo, hi in joining_group_lam:
        if lo == hi:
            s += f"                if c == 0x{lo:X} {{ return 0, 0 }}\n"
        else:
            s += f"                if c <= 0x{lo:X} || c >= 0x{hi:X} {{ return 0, 0 }}\n"
    s += f"""        case nextInfo == widthInfo_JOINING_GROUP_ALEF && isTransparentZeroWidth(c) :
...
def emit_module(
    ...
    """Outputs a Go file to `out_name` using table data from `tables`.
...
"""
        ...
        for variant in WidthState:
            if variant.is_carried():
                module.write(
                    f"    widthInfo_{variant.name} = 0b{variant.value:016b}\n"
                )
...
func isLigatureTransparent(c rune) bool {"""
        ...
        for lo, hi in ligature_transparent:
            if lo == hi:
                module.write(f"    if c == 0x{lo:X} {{ return true }}\n")
            else:
                module.write(f"    if c >= 0x{lo:X} && c <= 0x{hi:X} {{ return true }}\n")

        module.write("""    return false
}

func isSolidusTransparent(c rune) bool {
    if isLigatureTransparent(c) { return true }
    _, found := slices.BinarySearchFunc(
...
"""
        ...
        for msbs, i in emoji_presentation_idx:
            module.write(f"        case 0x{msbs:X}: idxOfLeaf = {i}\n")
...
"""
        ...
        for msbs, i in text_presentation_idx:
            module.write(f"        case 0x{msbs:X}: leaf = _TEXT_PRESENTATION_LEAF_{i}[:]\n")
...
"""
        ...
        for msbs, i in emoji_presentation_idx:
            module.write(f"        case 0x{msbs:X}: leaf = _EMOJI_MODIFIER_LEAF_{i}[:]\n")
...
"""
        ...
        for i, table in enumerate(tables):
            ...
            if table.bytes_per_row is None:
                module.write(
                    f"// Autogenerated. {subtable_count} sub-table(s). Consult [lookupWidth] for layout info.)\n"
                )
                module.write(
                    f"var _{table.name} = [{len(byte_array)}]byte{{"
                )
                ...
                module.write(
                    f"""
const _{table.name}_LEN int = {num_rows}
/// Autogenerated. {subtable_count} sub-table(s). Consult [lookupWidth] for layout info.
var _{table.name} = [_{table.name}_LEN][{table.bytes_per_row}]byte{{"""
                )
        ...
        module.write(
            f"""
var _NON_TRANSPARENT_ZERO_WIDTHS = [{len(non_transparent_zero_widths)}][2][3]byte{{
"""
        )
...
var _SOLIDUS_TRANSPARENT = [{len(solidus_transparent)}][2][3]byte{{
"""
        )
...
var _EMOJI_PRESENTATION_LEAVES = [{len(emoji_presentation_leaves)}][128]byte{{
"""
        )
        ...
        for leaf_idx, leaf in enumerate(text_presentation_leaves):
            module.write(
                f"""
var _TEXT_PRESENTATION_LEAF_{leaf_idx} = [{len(leaf)}][2]byte{{
"""
            )
            for lo, hi in leaf:
        ...
        for leaf_idx, leaf in enumerate(emoji_modifier_leaves):
            module.write(
                f"""
var _EMOJI_MODIFIER_LEAF_{leaf_idx} = [{len(leaf)}][2]byte{{
"""
            )
            for lo, hi in leaf:
                module.write(f"    {{0x{lo:02X}, 0x{hi:02X}}},\n")
...
#         s.chars()
#             .rfold((0, init), |(sum, nextInfo), c| -> (isize, WidthInfo) {{
#                 let (add, info) = width_in_str(c, nextInfo);
...
#         s.chars()
#             .rfold((0, init), |(sum, nextInfo), c| -> (isize, WidthInfo) {{
#                 let (add, info) = width_in_str_cjk(c, nextInfo);
#                 (sum.checked_add(isize::from(add)).unwrap(), info)
#             }})
#             .0
#     }}
...
#                 module.write(f"        widthInfo_{variant.name},\n")
