...
func (t Token) AsInt() (uint64, bool) {
	...
	any, present := t.Context().literals[t.raw]
...
// ensureProgress panics if this cursor has not advanced past the current mark.
func (c *Cursor) ensureProgress(mark CursorMark) {
	if mark.owner == nil {
		return
	}
	if mark.start == c.start && mark.idx == c.idx {
		panic(fmt.Sprintf("protocompile/ast: failed to make progress at offset %d; this is a bug in protocompile", c.start))
	}
}
